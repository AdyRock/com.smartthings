...
{
    static CLIENT_ID = Homey.env.SMARTTHINGS_CLIENT_ID || null;

    static CLIENT_SECRET = Homey.env.SMARTTHINGS_CLIENT_SECRET || null;
    ...
    getRefreshFingerprint( token = this.getToken() )
    {
        const activeSessionInfo = ( typeof this.homey?.app?.getActiveOAuth2SessionInfo === 'function' )
            ? this.homey.app.getActiveOAuth2SessionInfo()
            : null;
        ...
        return JSON.stringify( {
            sessionId: activeSessionInfo?.sessionId || null,
            configId: activeSessionInfo?.configId || null,
            appId: token?.app_id || null,
            installedAppId: token?.installed_app_id || null,
            expiresIn: token?.expires_in ?? null,
            obtainedAt: token?.obtained_at ?? token?.obtainedAt ?? null,
            accessTokenFp: this.getValueFingerprint( token?.access_token ),
            refreshTokenFp: this.getValueFingerprint( token?.refresh_token ),
            tokenClientIdFp: this.getValueFingerprint( token?.client_id ),
            resolvedClientIdFp: this.getValueFingerprint( resolvedClientId ),
            tokenClientSecretFp: this.getValueFingerprint( token?.client_secret ),
            resolvedClientSecretFp: this.getValueFingerprint( resolvedClientSecret ),
        } );
    }
    ...
    get _clientId()
    {
        return this.getResolvedClientCredential( this.__clientId, this._token?.client_id );
    }
    ...
    get _clientSecret()
    {
        return this.getResolvedClientCredential( this.__clientSecret, this._token?.client_secret );
    }
    ...
    getResolvedClientCredential( preferredValue, tokenValue )
    {
        if ( typeof preferredValue === 'string' )
        {
            const trimmedPreferred = preferredValue.trim();
            if ( trimmedPreferred && !SmartThingsOAuth2Client.isPlaceholderClientCredential( trimmedPreferred ) )
            {
        ...
        if ( typeof tokenValue === 'string' )
        {
            ...
            if ( trimmedToken && !SmartThingsOAuth2Client.isPlaceholderClientCredential( trimmedToken ) )
            {
                return trimmedToken;
            }
        }
    ...
    async onGetTokenByCode( { code } )
    {
        ...
        this.logRefreshDiagnostic( `token acquired by authorization code; fingerprint ${this.getRefreshFingerprint( this._token )}` );
    ...
    async onRefreshToken()
    {
        ...
        this.logRefreshDiagnostic( `starting refresh via ${this._tokenUrl} with token metadata ${this.getTokenDiagnostics( token )}; fingerprint ${this.getRefreshFingerprint( token )}` );
        ...
        if ( !response.ok )
        {
            ...
            this.logRefreshDiagnostic( `refresh failed with ${response.status} ${response.statusText || 'Unknown Error'}; fingerprint ${this.getRefreshFingerprint( token )}; response: ${responseDiagnostics}` );
        ...
        this._token.obtained_at = Date.now();
        this.logRefreshDiagnostic( `refresh succeeded; new token metadata ${this.getTokenDiagnostics( this._token )}; fingerprint ${this.getRefreshFingerprint( this._token )}` );
    ...
    async onShouldRefreshToken( { status } )
    {
        ...
        if ( status === 401 )
        {
            this.logRefreshDiagnostic( `received 401 from API; refresh will be attempted with token metadata ${this.getTokenDiagnostics( token )}; fingerprint ${this.getRefreshFingerprint( token )}` );
        }
